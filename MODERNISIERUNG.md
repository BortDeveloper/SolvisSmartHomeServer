...
## Stufe 4 — Sicherheit und Abhängigkeits-Hygiene

Läuft bewusst **parallel** zum 3.3-Rest (control.xml): kleine, unabhängige
Maßnahmen mit hohem Sicherheits-/Wartbarkeitsnutzen.

| # | Maßnahme | Status |
|---|---|---|
| 4.1 | **Mail-TLS: Zertifikatsprüfung aktiviert.** `mail.smtp.ssl.trust="*"` (Vertrauen in *jedes* Zertifikat, MITM-anfällig) entfernt — es gilt die JSSE-Standardprüfung gegen die System-Truststore-CAs. Öffentliche Provider (z. B. securesmtp.t-online.de) funktionieren unverändert; für private SMTP-Server mit Eigenzertifikat muss das Zertifikat in den Java-Truststore importiert werden (dokumentiert im CHANGELOG). | ✅ erledigt |
| 4.2 | **Automatische Abhängigkeits-Pflege:** Dependabot (Maven + GitHub-Actions, wöchentlich) + `maven-enforcer-plugin` (erzwingt Java ≥ 17 und Dependency-Konvergenz beim Build). | ✅ erledigt |
| 4.3 | **MQTT-Client-Evaluation:** Paho `mqttv3` 1.2.5 ist faktisch unmaintained; Umstieg auf Paho MQTT5 oder HiveMQ-Client prüfen. Die Sicht `MqttConnectionConfig` (3.3) kapselt die Verbindungs-Config bereits — der Austausch bleibt lokal in `Mqtt`/`MqttThread`. | geplant |
| 4.4 | **Threading modernisieren:** Eigenbau-Threads mit `wait/notify` (`MqttThread`, `BackupThread`, `MqttQueue`, `SolvisWorkers`) → `ScheduledExecutorService`; mittelfristig Java-21-Baseline (CI testet 21 bereits) und virtual threads. | geplant |
| 4.5 | **Testnetz-Ausbau + statische Analyse:** Unit-Tests für die jetzt über Sichten entkoppelte Kernlogik (HumanAccess-Statusmaschine, WatchDog, Distributor, TopicType-Routing); JaCoCo-Coverage, SpotBugs/Error Prone in der CI. | geplant |
| 4.6 | **CryptAes ehrlich machen:** hartkodierter Schlüssel + ECB ist Obfuskation, kein Schutz — dokumentieren und Alternative anbieten (Passwort aus Umgebungsvariable/Datei); deprecated Klartext-`password`-Attribut ausbauen. | geplant |
