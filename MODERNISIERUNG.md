...
| 3.3 | **XML-Binding ablösen**: die proprietäre `XMLLibrary` + die 67 `CreatorByXML`-Klassen durch **JAXB (Jakarta XML Binding)** ersetzen. **Sicherheitsnetz steht** (`BaseConfigParsingTest` pinnt das Ergebnis des aktuellen Parsers — der neue muss identische Werte liefern). **Offen: die eigentliche Konversion** — großer, in sich geschlossener Refactor (67 Klassen, XSD-Validierung neu verorten, die Sonderlogik der Creators via `afterUnmarshal` nachbilden). Als dedizierter Schritt geplant. | 🔄 Testnetz da, Konversion offen |
